...
- **Live polling + delete (added 2026-07-31, client request):** two deliberate, narrow exceptions to rules stated elsewhere in this document, added specifically for these four pages, not the panel generally.
  ...
  - **Not yet built - forced disconnect ("kick a connected device"), tested and confirmed NOT supported at Kelder (2026-07-31):** the client also asked for a way to forcibly disconnect an already-connected device from the panel. This is the Phase 2 "instant session revocation" item below (RFC 5176 CoA/Disconnect-Message). Tested directly against **three separate Kelder APs** from the Zonclave server, all with the identical result - `radclient -x <AP-IP>:3799 disconnect <secret>` with a real `User-Name`/`Acct-Session-Id` pulled from `radacct` got no reply after 3 retries (not a NAK - genuine silence): `192.168.1.217` (a closed test session), `192.168.1.242` (nmap-only check), and `192.168.1.169` (the actual NAS IP of a genuinely live `SancoUK1` session, confirmed via `radacct.nasipaddress` for that exact `acctsessionid` - the most rigorous run, since a real connected device could be watched and stayed online throughout). `sudo nmap -sU -p 3799` against `.217`/`.242` was ambiguous on repeat runs (`open|filtered` instead of the first run's `closed`) - a known UDP-scan artifact from ICMP rate-limiting, not a real state change; the `radclient` protocol-level test is the authoritative signal and was consistent across all three APs and both a synthetic and a real live session. This UniFi AP fleet's firmware does not run an RFC 5176 dynamic-authorization listener at all - there is no GUI toggle to find (a UniFi RADIUS-profile "Dynamic Authorization"/CoA setting was searched for and not confirmed to exist in the current Network app), and no amount of panel or FreeRADIUS-side code changes this without different AP hardware/firmware. **Force Disconnect stays unbuilt and off the roadmap for this hardware** - the Phase 1 workaround (manual kick from the UniFi controller, per Section 16 Phase 2 list below) is the answer, not a temporary state pending more testing. Revisit only if the client moves to APs/firmware with confirmed CoA support.
...
- **Instant session revocation on disable/delete** (identified 2026-07-23 during Section 21.1 test 5): disabling a PPSK correctly blocks re-authentication, but an already-connected device keeps its WPA2 session until it disconnects - RADIUS is only consulted at association time, so this is standard behavior, not a bug. Phase 1 workaround: kick the client manually in the UniFi controller after disabling. **Tested and blocked at Kelder (2026-07-31):** the planned Phase 2 approach (`PpskService`'s disable/delete path sends an RFC 5176 Disconnect-Message/CoA to the AP) requires the AP to run a dynamic-authorization listener on UDP 3799. Live tests against three separate Kelder APs (`radclient -x <AP-IP>:3799 disconnect <secret>`, real sessions pulled from `radacct` including one genuinely live session confirmed via `nasipaddress`) all got no reply after 3 retries - this AP fleet's firmware does not support CoA at all, and no UniFi GUI toggle to enable it was found either. See Section 16.6 for the full test detail. The manual-kick workaround stays the permanent answer for this hardware, not a placeholder; revisit only on different AP hardware/firmware with confirmed CoA support.
