...
- [x] VM OS: **Ubuntu 22.04 LTS** (confirmed running, not 24.04 as noted in earlier planning). PHP 8.3 was added via the ondrej/php PPA. Resolved 2026-07-16 by promoting `installer/install-ubuntu22.04.sh` to an officially tracked, supported installer alongside `install.sh` (Section 24.4), rather than treating 22.04 as an unsupported one-off.
...
- Target and pin the OS: **Ubuntu Server 24.04 LTS or 22.04 LTS** (updated 2026-07-16 - two officially supported versions, not one). `installer/install.sh` targets 24.04 (PHP 8.3 ships in the base repos, no third-party PPA needed) and `installer/install-ubuntu22.04.sh` targets 22.04 (PHP 8.3 added via the `ondrej/php` PPA, since 22.04's base repos only ship 8.1). Both are otherwise identical: same stages, same schema, same RADIUS/panel provisioning. Promoted 22.04 from a private local variant to an officially tracked installer on 2026-07-16 because it is what the Office SancoMedia Kelder deployment actually runs (Section 26) - the 24.04 pin had drifted from reality. Do not attempt to support every distro beyond these two, or one-click reliability breaks.
