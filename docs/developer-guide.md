...
- To split one list into several pre-filtered sidebar pages (see the Active PPSK Users / Stale Sessions / Inactive PPSK Users resources, all slices of the same `radacct` data as the "All Sessions" resource), don't fork the `Tables/*Table.php` file - add an optional scope parameter to its `configure()` method instead, and give each new resource its own thin `*Resource.php` + `Pages/List*.php` that calls `configure($table, $scope)`. This keeps the columns and filters defined in exactly one place, so the pages can't drift apart from each other over time. Group them in the sidebar with a shared `protected static string|UnitEnum|null $navigationGroup`.
- Every column on every table should be `->toggleable()` except one - the table's natural row-identity column (see CLAUDE.md Section 16.9) - so an admin can hide what they don't need without ever losing the ability to tell rows apart. Filament renders the toggle control and persists the choice per page automatically; no extra code beyond the `->toggleable()` calls themselves.
- **Sidebar position gotcha:** Filament always renders every ungrouped resource/page as one combined block, sorted at position -1, *before* any named `navigationGroup` - regardless of `navigationSort` values. In practice this means an item cannot sit strictly *between* an ungrouped item and a named group (or between two named groups in the wrong relative order) just by tweaking sort numbers; it has to join a real named group, and that group's position relative to other groups has to be set explicitly via `->navigationGroups([...])` on `AdminPanelProvider` (see Section 16.12's "System" group for a worked example - CLAUDE.md explains the full reasoning). Deciding "should this stay ungrouped (pins it to the very top) or join a named group (and which one)" is a real design choice worth making deliberately when adding a new top-level page, not an afterthought.
