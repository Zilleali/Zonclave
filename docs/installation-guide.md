...
This document is the narrative walkthrough. For the authoritative specification behind every decision here, see [CLAUDE.md](../CLAUDE.md) - if the two ever disagree, CLAUDE.md wins. For a bare command cheat sheet with no explanation, see [commands-reference.md](commands-reference.md). For a screen-by-screen walkthrough of using the panel day to day, see [user-guide.md](user-guide.md). For an architecture/contribution overview aimed at a developer picking up this codebase, see [developer-guide.md](developer-guide.md). For what's shipped and when, see [changelog.md](changelog.md).
...
| `docs/user-guide.md` | Screen-by-screen walkthrough of using the panel day to day. |
| `docs/developer-guide.md` | Architecture and contribution overview for a developer picking up this codebase. |
| `docs/changelog.md` | What shipped, and when - newest first, including the current version. |
...
A quick orientation - **[user-guide.md](user-guide.md) has the full screen-by-screen walkthrough**, this is just the map:
...
- **PPSK Groups** (`/admin/ppsk-groups`): the inventory list. Create a new PPSK (label is free text, VLAN from a dropdown, username and password each auto-generated by default with manual entry as an opt-in). Edit a group's label, VLAN, or RADIUS username directly, and optionally regenerate its password from the same modal. Every PSK is shown exactly once, with a copy-to-clipboard button.
...
| How do I actually use the panel day to day? | [user-guide.md](user-guide.md) |
| I'm a developer picking this codebase up for the first time | [developer-guide.md](developer-guide.md) |
