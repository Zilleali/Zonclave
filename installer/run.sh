...
#   Runs install-ubuntu22.04.sh, the one officially supported installer
#   (CLAUDE.md Section 24.4, ADR 0003). Any arguments after -- are
#   forwarded to it, e.g.:
...
# Everything after a literal "--" forwards to install-ubuntu22.04.sh; drop
# it from $@ before we consume our own (currently nonexistent) options.
INSTALL_ARGS=()
if [ "${1:-}" = "--" ]; then
  shift
  INSTALL_ARGS=("$@")
...
[ -f "${WORKDIR}/zonclave/install-ubuntu22.04.sh" ] || {
  echo "Decrypted payload is missing install-ubuntu22.04.sh. Corrupt or wrong payload file?" >&2
  exit 1
}

cd "${WORKDIR}/zonclave"
exec bash install-ubuntu22.04.sh "${INSTALL_ARGS[@]}"
